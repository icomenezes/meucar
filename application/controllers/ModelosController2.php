...
class ModelosController extends Zend_Controller_Action
{
	...
	public function ajaxAction(){
		...
		if($this->_getParam('fn') == 'getByModelo'){
			...
			// ===== MODIFICAÇÃO: BUSCA LOCAL AO INVÉS DE API =====
            ...
            if($this->getRequest()->isPost()){
                ...
                // ===== CORREÇÃO: Extrair ano do ano_modelo_fipe =====
                $ano_modelo_bruto = str_replace('32000', 'Zero', $_POST['ano_modelo_fipe']);
                
                // Se vier "2023 Gasolina", pega só "2023"
                $ano_modelo_parts = explode(' ', trim($ano_modelo_bruto));
                $arr['ano_modelo_fipe'] = $ano_modelo_parts[0];
            ...
            if($arrModelos == false) {
                ...
                $dados = array(
                    ...
                    'marca' => $arr['marca'],
                    'ano_modelo' => $arr['ano_modelo_fipe'], // ← Agora vem correto
                    'codigo' => $arr['codigo'],
                    'preco' => $arr['valor'],
                ...
                // ===== ADICIONAL: Verifica duplicata antes de inserir =====
                $existe = $dbModelos->fetchRow(
                    "marca = '".$dados['marca']."' 
                    AND modelo = '".$dados['modelo']."' 
                    AND ano_modelo = '".$dados['ano_modelo']."'"
                );
                ...
                if(!$existe) {
                    ...
                    $dbModelos->edt($existe['id'], $dados_update);
                    echo $existe['id'];
                }

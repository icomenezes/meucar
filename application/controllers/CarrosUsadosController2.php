...
*/
   ...
   public function buscaVeiculosAction() {
      ...
      $_POST['id_empresa'] = $this->_getParam('id_empresa');

      if ($this->_getParam('id_empresa')) {

         $this->view->arrEmpresa = $dbEmpresas->getEmpresa($this->_getParam('id_empresa'));
      }
      ...
      if ($_POST['tipo_busca'] == 1) {
         ...
         foreach ($arrVeiculo as $key => $veiculos) {
            ...
            $arrVeiculos[$veiculos['id']] = $veiculos;
         }
      } else {
   ...
   public function anuncioIrregularAction() {
      ...
      if ($this->getRequest()->isPost()) {
         ...
         $assunto = "Irregularidade no an�ncio site Meu Car - Enviado por " . $_POST['nome'];
         $corpo = "
			<html>
				<head>
					<meta content='text/html; charset=utf-8' http-equiv='Content-Type'>

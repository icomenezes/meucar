...
class Application_Model_DbTable_Agenda extends Zend_Db_Table_Abstract
{
	...
	public function _get($arr = array()){
		...
		if(isset($arr['data_inicial']) || isset($arr['data_final'])) {
			...
			$row->where("a.data BETWEEN '".$arr['data_inicial']."' AND '".$arr['data_final']."'");
		...
		}elseif(isset($arr['baixado']) && $arr['baixado']){
			
			$row->where("a.baixado = 1");
			
		}
	
		$row->order('a.data DESC');
	...
	public function ListaAgenda($arr = array()){
		...
		if(isset($arr['baixado']) && $arr['baixado'] != ""){
		
			$row->where("baixado = ".$arr['baixado']);
